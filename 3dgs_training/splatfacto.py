...
class SplatfactoModelConfig(ModelConfig):
    ...
    """Weight of depth supervision loss. Set >0 to enable depth loss."""
    depth_loss_type: Literal["l1", "huber"] = "huber"
    """Depth loss type."""
    depth_loss_huber_delta: float = 0.1
    """Huber delta (meters) for depth loss."""
    depth_loss_ramp_steps: int = 2000
    """Linearly ramp depth loss from 0 to weight over this many steps."""
    depth_min: float = 0.2
    """Minimum depth (meters) for depth supervision."""
    depth_max: float = 5.0
    """Maximum depth (meters) for depth supervision."""
    depth_confidence_min: int = 0
    """Minimum confidence level (0=low, 1=medium, 2=high) to use for depth supervision."""
...
class SplatfactoModel(Model):
    ...
    def get_outputs(self, camera: Cameras) -> Dict[str, Union[torch.Tensor, List]]:
        ...
        if self.config.depth_loss_weight > 0 or self.config.output_depth_during_training or not self.training:
            render_mode = "RGB+ED"
        else:
            render_mode = "RGB"
    ...
    def _get_depth_loss_weight(self) -> float:
        if self.config.depth_loss_weight <= 0:
            return 0.0
        if self.config.depth_loss_ramp_steps <= 0:
            return self.config.depth_loss_weight
        ramp = min(float(self.step) / float(self.config.depth_loss_ramp_steps), 1.0)
        return self.config.depth_loss_weight * ramp

    def _prepare_depth_targets(self, batch):
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None) -> Dict[str, torch.Tensor]:
        ...
        if self.training and self.config.depth_loss_weight > 0 and outputs.get("depth") is not None:
            depth_targets = self._prepare_depth_targets(batch)
            if depth_targets is not None:
                gt_depth, depth_mask = depth_targets
                ...
                if pred_depth is not None:
                    ...
                    if valid.any():
                        if self.config.depth_loss_type == "huber":
                            depth_loss = F.smooth_l1_loss(
                                pred_depth[valid],
                                gt_depth[valid],
                                beta=self.config.depth_loss_huber_delta,
                            )
                        else:
                            depth_loss = torch.abs(pred_depth[valid] - gt_depth[valid]).mean()
                        loss_dict["depth_loss"] = depth_loss * self._get_depth_loss_weight()
